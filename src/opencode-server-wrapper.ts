...
function isMessageWithParts(value: unknown): value is MessageWithParts {
    ...
    return !!obj.info && Array.isArray(obj.parts);
}
...
export class OpenCodeServer {
    ...
    static connect(baseUrl: string = DEFAULT_BASE_URL): OpenCodeServer {
        return new OpenCodeServer(baseUrl);
    }
    ...
    async createSession(
        title?: string,
        options?: { parentID?: string; directory?: string },
    ) {
        return unwrap(
    ...
    async prompt(
        ...
        const preferred = noReply
            ? reversed.find((message) => message.info.role === "user")
            : reversed.find((message) => message.info.role === "assistant");
        const selected = preferred ?? reversed[0];
        if (!selected) {
            throw new Error("Prompt completed but no message found");
        }
        return {
            info: selected.info,
    ...
    async replyPermission(
        ...
        const res = await fetch(
            `${this.baseUrl}/permission/${requestId}/reply`,
            {
                method: "POST",
    ...
    async rejectQuestion(requestId: string): Promise<boolean> {
        const res = await fetch(
            `${this.baseUrl}/question/${requestId}/reject`,
            {
                method: "POST",
            },
        );
    ...
    async searchText(pattern: string, options?: { directory?: string }) {
        return unwrap(
            this.client.find.text({ query: { pattern, ...options } }),
        );
    }

    async searchFiles(
        query: string,
        options?: { directory?: string; dirs?: "true" | "false" },
    ) {
    ...
    async searchSymbols(query: string, options?: { directory?: string }) {
        return unwrap(
            this.client.find.symbols({ query: { query, ...options } }),
        );
    }
    ...
    async setAuth(
        providerID: string,
        auth?: Parameters<OpencodeClient["auth"]["set"]>[0]["body"],
    ) {
        return unwrap(
            this.client.auth.set({ path: { id: providerID }, body: auth }),
        );
    }
    ...
    async addMcpServer(
        name: string,
        config:
            | import("@opencode-ai/sdk/client").McpLocalConfig
            | import("@opencode-ai/sdk/client").McpRemoteConfig,
    ) {
    ...
    async mcpAuthCallback(name: string, code: string) {
        return unwrap(
            this.client.mcp.auth.callback({ path: { name }, body: { code } }),
        );
    }
    ...
    async waitForIdle(
        ...
        throw new Error(
            `Session ${sessionId} did not become idle within ${timeout}ms`,
        );
    }
}
