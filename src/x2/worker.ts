...
function parseOptionalBypassModel(raw: string | undefined): string | null {
  ...
  try {
    ...
    logger.warn("invalid_bypass_model", {
      ...
      fallback: null,
      reason:
        error instanceof Error ? error.message : "Invalid bypass model format",
    });
    return null;
  }
}
...
function isTaskType(value: string): value is TaskType {
  return value === "omo_request" || value === "report" || isEq1TaskType(value);
}
...
function parseArgs(argv: string[]): WorkerOptions {
  const options: WorkerOptions = {
    ...
    source: "cli",
    bypassAgent: parseOptionalAgent(process.env.X2_AGENT_BYPASS_AGENT, null),
    bypassModel: parseOptionalBypassModel(process.env.X2_AGENT_BYPASS_MODEL),
    agentRoutingMode: parseRoutingMode(process.env.X2_AGENT_ROUTING, "auto"),
    simpleAgent: parseOptionalAgent(process.env.X2_AGENT_SIMPLE_AGENT, "spark"),
    ...
    retryMaxMs: 60000,
    baseUrl:
      firstNonEmpty(process.env.OPENCODE_BASE_URL) ?? "http://127.0.0.1:4996",
  };
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--x2-summarizer-agent":
        if (!next)
          throw new Error("--x2-summarizer-agent requires an agent name");
      ...
      case "--max-retries":
        ...
        break;
      case "--retry-base-ms":
        if (!next) throw new Error("--retry-base-ms requires milliseconds");
        options.retryBaseMs = Number(next);
        i++;
        break;
      case "--retry-max-ms":
        if (!next) throw new Error("--retry-max-ms requires milliseconds");
      ...
      case "--base-url":
        ...
        break;
      case "--bypass-agent":
        if (!next) throw new Error("--bypass-agent requires an agent name");
        options.bypassAgent = next;
        options.agentRoutingMode = "fixed";
        i++;
        break;
      case "--bypass-model":
        if (!next) throw new Error("--bypass-model requires a model value");
        options.bypassModel = parseBypassModel(next);
        i++;
        break;
      case "--agent-routing":
        if (!next)
          throw new Error("--agent-routing requires one of: auto | fixed");
        options.agentRoutingMode = parseRoutingMode(next, "auto");
        i++;
        break;
      case "--simple-agent":
        if (!next) throw new Error("--simple-agent requires an agent name");
        options.simpleAgent = parseOptionalAgent(next, null);
        i++;
        break;
      case "--complex-agent":
        if (!next) throw new Error("--complex-agent requires an agent name");
...
async function main() {
  ...
  const queue = new Queue(store, server, {
    ...
    taskAckNotifier: createTaskAckNotifierFromEnv(),
  });
