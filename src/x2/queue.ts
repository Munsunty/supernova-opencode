...
import type { TaskAckNotifier } from "../channels/task-ack";
...
export interface QueueOptions {
  ...
  taskAckNotifier?: TaskAckNotifier | null;
}
...
export class Queue {
  ...
  private taskAckNotifier: TaskAckNotifier | null;
  ...
  constructor(
    ...
    this.complexAgent = options.complexAgent
      ...
      : null;
    this.taskAckNotifier = options.taskAckNotifier ?? null;
  }
  ...
  async dispatchNext(): Promise<Task | null> {
    ...
    if (!task) return null;
    this.logTaskTransition(task, "pending", "running", "claimed_for_dispatch");
    if (task.type !== "report" && this.taskAckNotifier) {
      try {
        const stats = this.store.getStats();
        await this.taskAckNotifier.notifyStarted({
          task,
        ...
        logger.warn("task_started_ack_failed", {
    ...
    if (isEq1TaskType(task.type)) {
      try {
        if (!this.eq1Client) {
          throw new Error("Eq1 task cannot run: eq1Client is not configured");
        }
        ...
        const updated = this.store.updateTask(task.id, {
          ...
          error: null,
          runAgent: "eq1",
          runModel: formatProviderModel(result.provider, result.model),
          summaryAgent: "eq1-direct",
          summaryModel: formatProviderModel(result.provider, result.model),
          completedAt: this.now(),
        });
        this.logTaskTransition(
          updated,
    ...
    try {
      const telegramChatId = extractTelegramChatIdFromTaskSource(task.source);
    ...
    return JSON.stringify(
      {
        ...
        objective: "Classify task complexity for execution agent routing only.",
        policy: {
  ...
  private async resolveDispatchRoute(task: Task): Promise<DispatchRoute> {
    ...
    if (eq1Decision) {
      ...
      return {
        agent: selected,
        route: eq1Decision.level === "complex" ? "eq1_complex" : "eq1_simple",
        reason: eq1Decision.reason,
      };
    }
    ...
    return {
      agent: selected,
      route: complexity.level === "complex" ? "auto_complex" : "auto_simple",
  ...
  async finalizeRunning(): Promise<Task | null> {
    ...
    try {
      statuses = (await this.server.getSessionStatuses()) as SessionStatusMap;
    } catch (error) {
    ...
    try {
      ...
      if (!assistant) {
        ...
        assistant = this.findAssistantForTask(recentMessages, taskStartedAt);
  ...
  private extractEventMessageInfo(event: unknown): TaskEventMessageInfo | null {
  ...
  private handleFailure(
    ...
    const message = error instanceof Error ? error.message : String(error);
    const now = this.now();
    const retryDelayMs = shouldRetry ? this.getRetryDelayMs(attempts) : null;
    const retryAt =
