...
export function enqueueTelegramUpdate(
  ...
  const taskSource = encodeTelegramTaskSource(
    options.taskSource ?? DEFAULT_TASK_SOURCE,
    parsed.event.context.chatId,
  );
  const task = store.createTask(buildPrompt(parsed.event), taskSource);
  ...
  return {
    ...
    source,
    taskSource,
    chatId: parsed.event.context.chatId,
    messageId: parsed.event.context.messageId,
