...
function parseArgs(argv: string[]): PollerArgs {
  const args: PollerArgs = {
    ...
    dbPath: process.env.X2_DB_PATH ?? process.env.OPENCODE_X1_DB_PATH ?? null,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--source":
        ...
        break;
      case "--task-source":
        if (!next) throw new Error("--task-source requires a source label");
      ...
      case "--poll-interval":
        if (!next) throw new Error("--poll-interval requires milliseconds");
  ...
  while (running) {
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (!parsed.ok) {
          invalid += 1;
          const registerResult = enqueueTelegramUpdate(store, update, {
            source: options.source,
            taskSource: options.taskSource,
            parsed,
          });
        ...
        if (result.action === "enqueued") {
          ...
          try {
            await taskAckNotifier.notifyQueued({
              taskId: result.taskId ?? "",
              taskSource: result.taskSource ?? options.taskSource,
              pendingCount: result.pendingCount ?? 0,
              runningCount: result.runningCount ?? 0,
              chatId: result.chatId ?? parsed.event.context.chatId,
              replyToMessageId:
                result.messageId ?? parsed.event.context.messageId,
            });
            logger.info("x1_enqueue_ack_sent", {
              eventId: result.eventId,
              taskId: result.taskId,
              chatId: result.chatId ?? parsed.event.context.chatId,
            });
            ...
            logger.error("x1_enqueue_ack_failed", {
              eventId: result.eventId,
              taskId: result.taskId,
              chatId: result.chatId ?? parsed.event.context.chatId,
              error: message,
