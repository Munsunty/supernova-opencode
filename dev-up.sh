...
HOST_PORT="${X_OC_PODMAN_HOST_PORT:-${X_OC_PODMAN_OPENCODE_PORT:-4996}}"
DASHBOARD_HOST_PORT="${X_OC_PODMAN_DASHBOARD_HOST_PORT:-51234}"
CONTAINER_OPENCODE_PORT="${X_OC_PODMAN_CONTAINER_OPENCODE_PORT:-${X_OC_PODMAN_OPENCODE_PORT:-$HOST_PORT}}"
...
RUNTIME_SCRIPT="${X_OC_PODMAN_RUNTIME_SCRIPT:-/opt/opencode/src/index.ts}"
...
X1_MODE="${X_OC_PODMAN_X1_MODE:-both}"
...
X1_DIRECT_SOURCE="${X_OC_PODMAN_X1_DIRECT_SOURCE:-x1_chatbot}"
X1_DIRECT_AGENT="${X_OC_PODMAN_X1_DIRECT_AGENT:-spark}"
X1_DIRECT_BASE_URL="${X_OC_PODMAN_X1_DIRECT_BASE_URL:-http://127.0.0.1:${CONTAINER_OPENCODE_PORT}}"
X1_DIRECT_SESSION_MODE="${X_OC_PODMAN_X1_DIRECT_SESSION_MODE:-per-chat}"
X1_DIRECT_SYSTEM="${X_OC_PODMAN_X1_DIRECT_SYSTEM:-}"
X1_DIRECT_TOKEN="${X_OC_PODMAN_X1_DIRECT_TOKEN:-}"
X1_DIRECT_ALLOWED_USER_IDS="${X_OC_PODMAN_X1_DIRECT_ALLOWED_USER_IDS:-$X1_POLLER_ALLOWED_USER_IDS}"
X1_DIRECT_POLL_INTERVAL_MS="${X_OC_PODMAN_X1_DIRECT_POLL_INTERVAL_MS:-$X1_POLLER_POLL_INTERVAL_MS}"
X1_DIRECT_POLL_TIMEOUT_SEC="${X_OC_PODMAN_X1_DIRECT_POLL_TIMEOUT_SEC:-$X1_POLLER_POLL_TIMEOUT_SEC}"
X1_DIRECT_POLL_LIMIT="${X_OC_PODMAN_X1_DIRECT_POLL_LIMIT:-$X1_POLLER_POLL_LIMIT}"
X1_DIRECT_API_BASE="${X_OC_PODMAN_X1_DIRECT_API_BASE:-$X1_POLLER_API_BASE}"
X3_INTERVAL_MS="${X_OC_PODMAN_X3_INTERVAL_MS:-3000}"
X3_MAX_PROCESS="${X_OC_PODMAN_X3_MAX_PROCESS:-10}"
OPENCODE_BASE_URL="${X_OC_PODMAN_BASE_URL:-http://127.0.0.1:${CONTAINER_OPENCODE_PORT}}"
...
case "$X1_MODE" in
  ...
  *)
    echo "ERROR: invalid X_OC_PODMAN_X1_MODE=$X1_MODE (allowed: poller, webhook, direct, both, off)"
    exit 1
    ;;
esac
...
PODMAN_RUN_ARGS=(
  ...
  -e OPENCODE_X1_DIRECT_SCRIPT=/opt/opencode/src/x1/direct-chatbot.ts
  -e OPENCODE_X1_DIRECT_SOURCE="$X1_DIRECT_SOURCE"
  -e OPENCODE_X1_DIRECT_AGENT="$X1_DIRECT_AGENT"
  -e OPENCODE_X1_DIRECT_BASE_URL="$X1_DIRECT_BASE_URL"
  -e OPENCODE_X1_DIRECT_SESSION_MODE="$X1_DIRECT_SESSION_MODE"
  ...
  -e OPENCODE_RUNTIME_SCRIPT="$RUNTIME_SCRIPT"
...
if [ -n "$X1_DIRECT_TOKEN" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_TOKEN=$X1_DIRECT_TOKEN")
fi
...
if [ -n "$X1_DIRECT_ALLOWED_USER_IDS" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_ALLOWED_USER_IDS=$X1_DIRECT_ALLOWED_USER_IDS")
fi
if [ -n "$X1_DIRECT_POLL_INTERVAL_MS" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_POLL_INTERVAL_MS=$X1_DIRECT_POLL_INTERVAL_MS")
fi
if [ -n "$X1_DIRECT_POLL_TIMEOUT_SEC" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_POLL_TIMEOUT_SEC=$X1_DIRECT_POLL_TIMEOUT_SEC")
fi
if [ -n "$X1_DIRECT_POLL_LIMIT" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_POLL_LIMIT=$X1_DIRECT_POLL_LIMIT")
fi
if [ -n "$X1_DIRECT_API_BASE" ]; then
  PODMAN_RUN_ARGS+=(-e "OPENCODE_X1_DIRECT_API_BASE=$X1_DIRECT_API_BASE")
fi
...
if [ "$X1_ENABLED" = "1" ]; then
  ...
  elif [ "$X1_MODE" = "both" ]; then
    if [ -n "$X1_DIRECT_TOKEN" ]; then
      _x1_direct_token_state="set"
    else
      _x1_direct_token_state="empty"
    fi
    echo "X1 mode:      both (poller source=$X1_POLLER_SOURCE + direct source=$X1_DIRECT_SOURCE agent=$X1_DIRECT_AGENT base=$X1_DIRECT_BASE_URL direct_token=$_x1_direct_token_state)"
  elif [ "$X1_MODE" = "direct" ]; then
    echo "X1 mode:      direct (source=$X1_DIRECT_SOURCE agent=$X1_DIRECT_AGENT base=$X1_DIRECT_BASE_URL)"
  else
