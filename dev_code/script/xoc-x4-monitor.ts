...
import { Store, type Task, type TaskStatus } from "../../src/x2/store";
...
function loadOpencodeLogSummary(pathInput?: string): LogSummary {
  ...
  for (const line of lines) {
    ...
    const isStatusPollLine = line.includes(
      "service=server method=GET path=/session/status request",
    );
    const sessionId = parseSessionId(line);
    if (!sessionId) {
      if (isStatusPollLine && lastSessionId) {
        const lastStats = sessions.get(lastSessionId);
        if (lastStats) {
          if (ts) lastStats.lastEventAt = ts;
          lastStats.statusPolls += 1;
        }
      }
      continue;
    }
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--source":
        ...
        break;
      case "--opencode-log":
        if (!next) throw new Error("--opencode-log requires a file path");
...
function printHelp() {
  console.log(`Usage:
  bun run .devserver/dev_code/script/xoc-x4-monitor.ts [options]

Options:
  --db <path>            state.db path (default: .devserver/src/x2/store default path)
