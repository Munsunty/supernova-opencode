 ...
 * @example
 * ```ts
 * import { captureDashboard } from "./.devserver/dashboard-screenshot"
 * const png = await captureDashboard()
 * ```
 *
 * @example CLI
 * ```bash
 * bun run .devserver/dashboard-screenshot.ts
 * bun run .devserver/dashboard-screenshot.ts --url http://127.0.0.1:51234 --output ./shot.png
 * ```
 */
...
if (import.meta.main) {
  ...
  const output =
    getArg("--output") ?? resolve(SCREENSHOTS_DIR, `dashboard-${timestamp}.png`)
