/**
 * x1-webhook — Telegram webhook 수신 HTTP 서버 엔트리포인트
 *
 * Usage:
 *   bun run .devserver/dev_code/script/x1-webhook.ts --port 5100 --path /webhook
 */

import { resolve } from "node:path";
import { createTelegramWebhookServer } from "../../src/x1/server";
...
function parseArgs(argv: string[]): WebhookArgs {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (arg) {
            case "--host":
                if (!next) throw new Error("--host requires host");
                options.host = next;
                i++;
                break;
            case "--port":
                ...
                break;
            case "--path":
                if (!next) throw new Error("--path requires a path");
                options.path = next;
                i++;
                break;
            case "--source":
...
function printHelp() {
    console.log(`Usage:
  bun run .devserver/dev_code/script/x1-webhook.ts [options]
