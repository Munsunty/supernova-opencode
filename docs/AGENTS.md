...
| HOMSA 위치 정의(전체 시스템 구조~Dₚ 조건) | `.devserver/docs/HOMSA.md`, `.devserver/src/opencode-server-wrapper.ts`, `.devserver/src/x2/*.ts`, `.devserver/src/x3/*.ts`, `.devserver/src/x4/*.ts`, `.devserver/src/eq1/*.ts`, `.devserver/src/control-plane/*` | X/Eq/D/W/L/L' 정의와 실제 구현 정합성 |
| 아키텍처 결정사항 | `.devserver/dev-up.sh`, `.devserver/entrypoint.sh`, `.devserver/run-sync/opencode.json`, `.devserver/.env.example`, `.devserver/docs/control-plane.md`, `.devserver/src/control-plane/*` | 확정/미확정 상태가 코드 기준과 일치하는지 |
...
| 패키지 구성 | `.devserver/docs/project-structure.md`, `.devserver/README.md`, `.devserver/docs/control-plane.md` | 현재 구현 구조와 목표 구조의 차이 갱신 |
| 기동 방식 (현재 동작) | `package.json`, `.devserver/dev-up.sh`, `.devserver/dev-doctor.sh`, `.devserver/dev-smoke.sh`, `.devserver/docs/control-plane.md` | 실제 실행 커맨드와 문서 예시 일치 여부 |
...
| Dₚ 사용 시나리오 | `.devserver/docs/isolation.md`, `.devserver/dev-up.sh`, `.devserver/opencode.json`, `.devserver/docs/control-plane.md` | 격리/포트/멀티 프로젝트 시나리오 정합성 |
...
| OpenCode 격리 관련 환경변수 | `.devserver/.env.example`, `.devserver/dev-up.sh`, `.devserver/entrypoint.sh`, `.devserver/docs/control-plane.md` | env 이름/기본값/설명 동기화 |
| 현재 런타임 상태 | `.devserver/docs/operations.md`, `.devserver/docs/control-plane.md` | 운영 상태/차단요인/운영 이력 갱신 시점 |
...
15. **control-plane은 신규 Core Xₙ이 아니라 운영용 프론트/백엔드 Wrapper surface**
    - HOMSA `§3` 기준으로 같은 `S` 안의 `X_front`/`X_backend` 조합으로 해석한다
    - 프론트의 독립 Lₙ은 source 선택, view 전환, 폼 상태, session drill-down 같은 UI 상태 관리다
    - 프론트에서 백엔드로 넘어가는 API 호출은 W이며, `fetch`를 컴포넌트/화면 상태와 분리해 취급한다
    - 목적은 Telegram 없는 운영 채널과 multi-devserver 관측이며, X₂/X₃/X₄/X_oc의 Core 정의를 바꾸지 않는다

16. **control-plane 데이터 접근 규칙은 기존 Dₚ/API 원칙을 그대로 따른다**
    - OpenCode 네이티브 데이터는 HTTP API / SDK wrapper로 조회한다
    - 자체 운영 데이터는 `state.db`와 `metrics_events`를 직접 사용한다
    - `opencode.db` 직접 조회를 control-plane에서도 금지한다
    - dashboard는 read-first, redaction-first 운영 표면으로 유지한다
...
5. **control-plane + web channel** → **구현 완료**
   - 엔트리포인트: `.devserver/src/control-plane/server.ts`
   - 구성: registry loader, summary adapter, static frontend(`public/`)
   - 기능: source 선택, project health / queue / interaction / metrics 요약, X4 recent decisions, session detail 조회, web thread 기반 X₂ enqueue
   - registry 기본 단일 source는 현재 저장소를 자동 등록하며, OpenCode base URL은 `CONTROL_PLANE_OPENCODE_BASE_URL` > `X_OC_PODMAN_X1_DIRECT_BASE_URL` > `OPENCODE_BASE_URL` > `http://127.0.0.1:4996` 우선순위를 따른다
...
4. **control-plane 후속 범위**
   - 현재 dashboard/API + enqueue surface는 구현 완료
   - web thread 응답/리포트 전달 규칙은 미정
   - multi-devserver registry live reload는 미정
   - agent collaboration view / planner-builder-reviewer orchestration 표면은 후속 범위
...
- 운영 표면 모듈: `src/control-plane/` (API + dashboard), `control-plane/` (projects registry)
...
# control-plane registry 확인
bun run control:check

# control-plane 운영 화면 기동
bun run control:plane
# (선택) 바인딩 조정:
# CONTROL_PLANE_HOST=127.0.0.1 CONTROL_PLANE_PORT=4310 bun run control:plane
```
...
- `X_front`: control-plane static UI (`src/control-plane/public/*`)
- `X_backend`: control-plane API / registry / summary adapter (`src/control-plane/*.ts`)
- 프론트 내부 상태 전환은 Lₙ, 프론트 → 백엔드 API 호출은 W로 분리
- 이 표면은 Core X₁~X₄/X_oc를 대체하지 않고 관측/운영 채널로만 붙는다
...
- `./control-plane.md` — control-plane/dashboard 목적, API, 운영 경로
...
- **oh-my-opencode-dashboard**: 읽기 중심, redaction-first dashboard 레퍼런스
...
| `CONTROL_PLANE_PROJECTS_FILE` | control-plane registry 파일 경로 | multi-devserver source 등록 |
| `CONTROL_PLANE_OPENCODE_BASE_URL` | control-plane 기본 source용 OpenCode URL override | 단일 source fallback |
| `CONTROL_PLANE_HOST` | control-plane bind host | 운영 UI/API 바인딩 |
| `CONTROL_PLANE_PORT` | control-plane bind port | 운영 UI/API 바인딩 |
...
- `./operations.md`
- control-plane 기능/로드맵은 `./control-plane.md`를 기준으로 관리한다
